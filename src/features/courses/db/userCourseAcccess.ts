...
export async function revokeUserCourseAccess(
  ...
  const validPurchases = await trx.query.PurchaseTable.findMany({
    ...
    with: {
      product: {
        with: { CourseProductTable: { columns: { courseId: true } } },
      },
    },
  });
  ...
  const validCourseIds = validPurchases.flatMap((p) =>
    p.product.CourseProductTable.map((cp) => cp.courseId)
  );

  const removeCourseIds = refundPurchase.CourseProductTable.flatMap(
    (cp) => cp.courseId
