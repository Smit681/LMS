...
export async function createSection(
  ...
  const { success, data } = sectionSchema.safeParse(unsafeData); // Validate the data

  if (!success) {
    return { error: true, message: "Invalid section date" };
  }
  ...
  if (!canCreateCourseSections(await getCurrentUser())) {
    return { error: true, message: "Section Creation not allowed" };
  }
