...
import { insertPurchase } from "@/features/purchses/db/purchases";
...
async function processStripeCheckout(checkoutSession: Stripe.Checkout.Session) {
  ...
  const courseIds = product.CourseProductTable.map((cp) => cp.courseId);
...
function getProduct(id: string) {
  return db.query.ProductTable.findFirst({
    ...
    with: {
      CourseProductTable: { columns: { courseId: true } },
    },
  });
}
