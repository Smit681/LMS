...
import { getPurchaseIdTag } from "@/features/purchses/db/cache";
...
function formatePrice(amountInDoller: number) {
  if (amountInDoller > 0) return `$${amountInDoller}`;
  if (amountInDoller == 0) return `$0`;
  else {
    return `-$${amountInDoller - 2 * amountInDoller}`;
  }
}
