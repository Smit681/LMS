...
import RevenueChart from "@/features/purchses/components/RevenueChart";
import { getPurchaseGlobalTag } from "@/features/purchses/db/cache";
  ...
  return (
    <div className="px-10 my-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 md:grid-cols-4 gap-4">
        <StatCard title="Net Sales">${netSales}</StatCard>
        <StatCard title="Refunded Sales">${totalRefunds}</StatCard>
...
async function getData() {
  const rawData = await db
    .select({
      date: PurchaseTable.createdAt,
      amount: PurchaseTable.pricePaidInCents,
      refunded: isNotNull(PurchaseTable.refundedAt),
    })
    .from(PurchaseTable)
    .orderBy(PurchaseTable.createdAt);
  let revenueSum = 0;
  const data = rawData.map((obj) => {
    if (obj.refunded == true) revenueSum -= obj.amount;
    else revenueSum += obj.amount;
    return {
      date: formatDate(obj.date),
      revenue: revenueSum > 0 ? revenueSum / 100 : 0,
    };
  });
  return data;
}
